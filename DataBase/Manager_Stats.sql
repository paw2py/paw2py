...
at line 16 above there should not be where conditon .
...
WHEN MATCHED THEN
	UPDATE SET mgr_stat.manager = upd.manager,
			   ...
			   --we can use decode to avoide update records that are not changed
			   --where decode(mgr_stat.manager,upd.manager,0,1) = 1
WHEN NOT MATCHED THEN
