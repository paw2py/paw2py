...
def count_of_nodes(root, queries, s):
  ...
  while qarr:
    par = qarr[0]
    ndstrdist[par.val] = s[par.val-1]
    if par.children:      
      for i in range(len(par.children)):
        newqarr.append(par.children[i])
        
        ndstrdist[par.val] += s[par.children[i].val-1]
        #print('chld',par.children[i].val)
    qarr.pop(0)
    if not qarr:
      qarr = newqarr.copy()
      newqarr.clear()
  ##chek for queries now
  #print('dist', ndstrdist)
  ndstrdist[root.val] = s
  for q in queries:
    if q[0] in ndstrdist:
      result.append(ndstrdist[q[0]].count(q[1]))
